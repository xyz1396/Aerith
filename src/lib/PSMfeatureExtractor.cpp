...
size_t PSMfeatureExtractor::binarySearchPeak(const Scan *mScan, double Mz,
                                             int charge) {
    ...
    while (low <= high) {
        ...
        if (diff <= mzTolerance) {
            ...
            // if (mScan->charge[mid] == charge && mScan->intensity[mid] >
            // currentIntensity)
...
void PSMfeatureExtractor::filterIsotopicPeaks(
    ...
    for (size_t i = 1; i < isotopicPeaks.size(); i++) {
        intensityDiff =
            isotopicPeaks[i].intensity - isotopicPeaks[i - 1].intensity;
        if (lastIntensityDiff > 0 && intensityDiff < 0)
            vertexIXs.push_back(i - 1);
        lastIntensityDiff = intensityDiff;
    }
    if (lastIntensityDiff > 0) vertexIXs.push_back(isotopicPeaks.size() - 1);
    ...
    for (size_t i = 0; i < vertexIXs.size(); i++) {
        MZdiff =
            std::abs(isotopicPeaks[vertexIXs[i]].mz - calculatedPrecursorMZ);
    ...
    while (currentPeakIX >= 0) {
        intensityDiff = isotopicPeaks[currentPeakIX].intensity -
                        isotopicPeaks[currentPeakIX + 1].intensity;
        if (intensityDiff > 0) {
            isotopicPeaks.erase(isotopicPeaks.begin(),
                                isotopicPeaks.begin() + currentPeakIX + 1);
    ...
    while (currentPeakIX < (int)isotopicPeaks.size()) {
        intensityDiff = isotopicPeaks[currentPeakIX].intensity -
                        isotopicPeaks[currentPeakIX - 1].intensity;
        if (intensityDiff > 0) {
            isotopicPeaks.erase(isotopicPeaks.begin() + currentPeakIX,
                                isotopicPeaks.end());
            break;
        }
        currentPeakIX++;
...
std::vector<isotopicPeak> PSMfeatureExtractor::findIsotopicPeaks(
    ...
    for (int i = 0; i < 3; i++) {
        peakIX =
            binarySearchPeak(MS1Scan, observedPrecursorMZ, precursorCharge);
    ...
    if (peakIX != std::numeric_limits<size_t>::max()) {
        ...
        isotopicPeaks.push_back(
            {MS1Scan->mz[peakIX], precursorCharge, MS1Scan->intensity[peakIX]});
        ...
        std::sort(isotopicPeaks.begin(), isotopicPeaks.end(),
                  [](const isotopicPeak &a, const isotopicPeak &b) {
                      return a.mz < b.mz;
                  });
    }
...
double PSMfeatureExtractor::getSIPelementAbundanceFromMS1(
    const std::string &nakePeptide,
...
std::pair<int, double> PSMfeatureExtractor::getMassWindowShiftAndError(
    const double observedPrecursorMass, const double calculatedPrecursorMass) {
...
double PSMfeatureExtractor::getMS2IsotopicAbundance(
    const std::string &searchName) {
    if (searchName == "SE") return 1.07;
...
void PSMfeatureExtractor::loadFT1(const std::string &FTfileBasePath) {
    ...
    for (size_t i = 0; i < FT1Scans.size(); i++) {
        scanNumerFT1ScanMap.insert(
            {FT1Scans[i].scanNumber, FT1Scans.data() + i});
    }
}
...
void PSMfeatureExtractor::loadFT2(const std::string &FTfileBasePath) {
    ...
    for (size_t i = 0; i < FT2Scans.size(); i++) {
        scanNumerFT2ScanMap.insert(
            {FT2Scans[i].scanNumber, FT2Scans.data() + i});
    }
}

void PSMfeatureExtractor::loadFT1FT2fileParallel(
    const std::string &FTfileBasePath) {
...
void PSMfeatureExtractor::extractFeaturesOfEachPSM() {
    ...
    for (size_t i = 0; i < mSipPSM->isotopicPeakss.size(); i++) {
        ...
        mSipPSM->MS2IsotopicAbundances[i] =
            getMS2IsotopicAbundance(mSipPSM->searchNames[i]);
        mSipPSM->isotopicAbundanceDiffs[i] = mSipPSM->MS1IsotopicAbundances[i] -
                                             mSipPSM->MS2IsotopicAbundances[i];
    }
}

void PSMfeatureExtractor::extractPSMfeature(const std::string &Spe2PepFilePath,
                                            const int topN,
                                            const std::string &ftFilepath) {
    mSpe2PepFileReader.readSpe2PepFilesScansTopPSMsFromEachFT2Parallel(
        Spe2PepFilePath, topN);
    for (size_t i = 0; i < mSpe2PepFileReader.FT2s.size(); i++) {
        // mExtractor.loadFT1FT2fileParallel(ftFilepath + "/" +
        // mSpe2PepFileReader.FT2s[i]);
        ...
        mSipPSM->isotopicPeakss =
            std::vector<std::vector<isotopicPeak>>(mSipPSM->scanNumbers.size());
        mSipPSM->isotopicPeakNumbers =
            std::vector<int>(mSipPSM->scanNumbers.size());
        mSipPSM->MS1IsotopicAbundances =
            std::vector<double>(mSipPSM->scanNumbers.size());
        mSipPSM->MS2IsotopicAbundances =
            std::vector<double>(mSipPSM->scanNumbers.size());
        mSipPSM->isotopicAbundanceDiffs =
            std::vector<double>(mSipPSM->scanNumbers.size());
        mSipPSM->peptideLengths = std::vector<int>(mSipPSM->scanNumbers.size());
        mSipPSM->missCleavageSiteNumbers =
            std::vector<int>(mSipPSM->scanNumbers.size());
        mSipPSM->diffScores = std::vector<float>(mSipPSM->scanNumbers.size());
        mSipPSM->mzShiftFromisolationWindowCenters =
            std::vector<double>(mSipPSM->scanNumbers.size());
        mSipPSM->isotopicMassWindowShifts =
            std::vector<int>(mSipPSM->scanNumbers.size());
        mSipPSM->massErrors = std::vector<double>(mSipPSM->scanNumbers.size());
        mSipPSM->precursorIntensities =
            std::vector<double>(mSipPSM->scanNumbers.size(), 0);
...
#pragma omp parallel for
    for (size_t i = 0; i < mSpe2PepFileReader.FT2s.size(); i++) {
        ...
        // init vectors for features to be extracted;
        mExtractor.mSipPSM->isotopicPeakss =
            std::vector<std::vector<isotopicPeak>>(
                mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->isotopicPeakNumbers =
            std::vector<int>(mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->MS1IsotopicAbundances =
            std::vector<double>(mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->MS2IsotopicAbundances =
            std::vector<double>(mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->isotopicAbundanceDiffs =
            std::vector<double>(mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->peptideLengths =
            std::vector<int>(mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->missCleavageSiteNumbers =
            std::vector<int>(mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->PTMnumbers =
            std::vector<int>(mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->diffScores =
            std::vector<float>(mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->mzShiftFromisolationWindowCenters =
            std::vector<double>(mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->isotopicMassWindowShifts =
            std::vector<int>(mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->massErrors =
            std::vector<double>(mExtractor.mSipPSM->scanNumbers.size());
        mExtractor.mSipPSM->precursorIntensities =
            std::vector<double>(mExtractor.mSipPSM->scanNumbers.size(), 0);
        mExtractor.extractFeaturesOfEachPSM();
    }
}

void PSMfeatureExtractor::extractPSMfeatureParallel(
    const std::string &Spe2PepFilePath, const int topN,
    const std::string &ftFilepath, const int threadNumber) {
    mSpe2PepFileReader.readSpe2PepFilesScansTopPSMsFromEachFT2Parallel(
        Spe2PepFilePath, topN);
...
void PSMfeatureExtractor::extractPSMfeatureParallel(
    const std::string &targetPath, const std::string &decoyPath, const int topN,
    const std::string &ftFilepath, const int threadNumber) {
    mSpe2PepFileReader
...
void PSMfeatureExtractor::writeTSV(const std::string &fileName) {
    ...
    for (size_t i = 0; i < sipPSMs.size(); i++) {
        for (size_t j = 0; j < sipPSMs[i].fileNames.size(); j += chunkSize) {
            for (size_t k = j;
                 k < std::min(j + chunkSize, sipPSMs[i].fileNames.size());
                 ++k) {
                ss << sipPSMs[i].fileNames[k] << "\t"
                   ...
                   << sipPSMs[i].WDPscores[k] << "\t" << sipPSMs[i].ranks[k]
    ...
    for (size_t i = 0; i < sipPSMs.size(); i++) {
        for (size_t j = 0; j < sipPSMs[i].fileNames.size(); j += chunkSize) {
            for (size_t k = j;
                 ...
                 ++k) {
                ss << sipPSMs[i].fileNames[k] << "."
                   << sipPSMs[i].scanNumbers[k] << "." << sipPSMs[i].ranks[k]
                   << "\t";
                ss << (sipPSMs[i].isDecoys[k] ? -1 : 1) << "\t";
                ss << sipPSMs[i].scanNumbers[k] << "\t"
                   << sipPSMs[i].calculatedParentMasses[k] << "\t"
                   << sipPSMs[i].retentionTimes[k] << "\t"
                   << sipPSMs[i].ranks[k] << "\t" << sipPSMs[i].parentCharges[k]
                ...
                if (doProteinInference) {
                    ...
                    if (peptideSeq.back() == ']') peptideSeq.push_back('n');
                ...
                if (doProteinInference) {
                    // for percolator protein inference format
                    proteinName = proteinName.substr(
                        1, sipPSMs[i].proteinNames[k].length() - 2);
